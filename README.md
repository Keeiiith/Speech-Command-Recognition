...
1. 
**Audio Input:** Captured real-time or from the Google Speech dataset.


2. 
**Spectrogram Conversion:** Raw audio is processed via STFT into a Mel-Spectrogram.


3. 
**CNN Inference:** The model outputs probability scores for each supported command.


4. 
**RL Thresholding:** The RL agent evaluates the confidence score and decides if the threshold is met based on learned costs.


5. 
**NLP Verification:** If "Play" is detected, the system performs a metadata search to finalize the music request.
